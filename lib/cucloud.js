...
// plugins
CUCloud.plugins = {}
CUCloud.plugins.cnamer = require('./plugins/cnamer.js')
...
CUCloud.setPluginSettings = (pluginName, pluginSettings) => {
  return new Promise(function (resolve, reject) {
    ...
    pluginConfig.save(function (err) {
      ...
      log.info('Saved settings for ' + pluginName)
      return resolve({})
    })
  })
}
...
CUCloud.getAllConfig = () => {
  return new Promise(function (resolve, reject) {
    var callback = function (err, response) {
      if (err) log.error(err)
      var items = {}
      for (var i = 0; i < response.Items.length; i++) {
        var settings = response.Items[i].attrs.settings
        var plugin = response.Items[i].attrs.plugin
        items[plugin] = settings
      }
      return resolve(items)
    }

    return ConfigTable.query(CUCloud.config.profileName).loadAll().exec(callback)
  })
}
